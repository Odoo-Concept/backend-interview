...
class GroceryList(models.Model):
    ...
    def _compute_completed_items_count(self):
        for record in self:
            record.completed_items_count = len(
                record.item_ids.filtered(lambda x: x.is_purchased)
            )
        ...
        for record in self:
            if record.item_count > 0:
                record.progress_percentage = (
                    record.completed_items_count / record.item_count
                ) * 100
            else:
        ...
        for record in self:
            if record.responsible_id != self.env.user:
                raise ValidationError(
                    _("Solo el responsable de la lista puede realizar esta acción.")
                )

    def write(self, vals):
        """Sobreescribe write para comprobar que el usuario actual
        sea el responsable de cada lista.
        """
        self._check_responsibility()
        return super().write(vals)

    def unlink(self):
        """Sobreescribe unlink para comprobar que el usuario actual
        sea el responsable de cada lista.
        """
        self._check_responsibility()
        return super().unlink()
    ...
    def action_set_in_progress(self):
        ...
        if not self.item_ids:
            raise ValidationError(
                _('No se puede cambiar a "En Progreso" una lista sin productos.')
            )
    ...
    def action_set_completed(self):
        ...
        # Comprobar si el usuario que intenta realizar la acción es el responsable
        # de la lista
        self._check_responsibility()
        ...
        if not self.item_ids:
            raise ValidationError(
                _("No se puede marcar como completada una lista sin productos.")
            )
        ...
        if unpurchased_items:
            raise ValidationError(
                _(
                    "No se puede marcar como completada una lista que tiene "
    ...
    def action_add_item(self):
        ...
        return {
            ...
            "views": [
                (
                    self.env.ref("grocery_list.view_grocery_list_item_form_compact").id,
                    "form",
                )
            ],
            "target": "new",

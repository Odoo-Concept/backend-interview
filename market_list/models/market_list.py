...
class MarketList(models.Model):
    ...
    product_ids = fields.One2many(
        "market.list.product", "market_list_id", string="Products"
    )
    date = fields.Date(default=fields.Date.context_today)
    is_confirmed = fields.Boolean(
        string="Purchase Confirmed", default=False, store=True
    )
    ...
    def action_confirm_purchase(self):
        for record in self:
            if record.state != "done":
                raise UserError(
                    _("You can only confirm purchases when the list is completed.")
                )
            if record.is_confirmed:
                raise UserError(_("The purchase has already been confirmed."))

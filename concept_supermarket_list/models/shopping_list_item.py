...
class ShoppingListItem(models.Model):
    ...
    list_state = fields.Selection(
        related="shopping_list_id.state", string="List State", store=True, readonly=True
    )
    ...
    def unlink(self):
        ...
        for record in self:
            ...
            if not (is_responsible or is_admin):
                raise AccessError(_("Only the list responsible can delete items."))
    ...
    def _check_permission_to_modify(self):
        ...
        for record in self:
            ...
            if not (is_responsible or is_admin):
                raise AccessError(_("Only the list responsible can modify items."))

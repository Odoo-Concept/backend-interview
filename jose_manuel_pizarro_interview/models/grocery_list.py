...
class GroceryList(models.Model):
    ...
    user_responsible_id = fields.Many2one(
        "res.users", "Responsible", default=lambda self: self.env.user, readonly=True
    )
    state = fields.Selection(
        GROCERY_LIST_STATE, "Status", readonly=True, copy=False, default="draft"
    )
    ...
    def action_set_in_progress(self):
        ...
        if self.state != "draft":
            raise ValidationError(
                self.env._(
                    "Your grocery list must be in Draft state in order to "
                    "change to In "
                    "Progress State"
                )
            )
    ...
    def action_set_done(self):
        ...
        if not self.state == "in_progress":
            raise ValidationError(
                self.env._(
                    "Your grocery list must be in Progress state in order to "
                    "complete "
                    "it"
                )
            )
        if not all(self.grocery_lines.mapped("is_purchased")):
            raise ValidationError(
                self.env._(
                    "You need to purchase all the items in order to "
                    "complete the "
                ...
                + "\n".join(
                    ...
                    for name in self.grocery_lines.filtered(

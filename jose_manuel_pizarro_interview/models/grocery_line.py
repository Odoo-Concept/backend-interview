...
class GroceryLine(models.Model):
    ...
    can_edit_is_purchased = fields.Boolean(string='Can Edit Is Purchased',compute='_compute_can_edit_is_purchased')
    grocery_list_id = fields.Many2one('grocery.list', string='Grocery')
    user_responsible_id = fields.Many2one(related='grocery_list_id.user_responsible_id', string='Responsible')
    ...
    def _compute_can_edit_is_purchased(self):
        for line in self:
            line.can_edit_is_purchased = line.user_responsible_id.id == self.env.user.id and line.state == 'in_progress'
